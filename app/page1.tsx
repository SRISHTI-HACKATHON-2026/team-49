  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#FFFFFF] px-4 py-5 font-sans text-[#1A1A1A]">
      ...
      <div className="absolute inset-0 z-0 flex pointer-events-none">
        <div className={`relative h-full flex items-center justify-center pointer-events-auto transition-all duration-1000 ease-out ${(!user && view === "landing") ? "w-full opacity-100" : "w-full lg:w-[calc(100%-700px)] opacity-40"
          }`}>
          <Spline
            scene={
        ...
        <section className={`pointer-events-auto relative z-10 mx-2 md:ml-auto md:mr-8 lg:mr-16 mt-2 md:mt-0 flex h-[calc(100dvh-1rem)] md:h-[92vh] w-[calc(100%-1rem)] md:w-full max-w-[700px] flex-col overflow-hidden rounded-3xl md:rounded-[28px] border-2 border-[#FF8C42]/30 bg-white/95 backdrop-blur-3xl shadow-[0_20px_70px_-10px_rgba(255,140,66,0.25)] ring-4 md:ring-[10px] ring-[#FF8C42]/5 transition-all duration-1000 ease-in-out`}>
          ...
          {demoMode && (
            <div className="h-[2px] w-full bg-slate-100 relative z-50">
              <div
                className={`absolute left-0 top-0 h-full transition-all duration-1000 ease-in-out ${currentStress === "LOW" ? "w-1/3 bg-gradient-to-r from-emerald-400 to-emerald-300" :
          ...
          {autoReportBanner && (
            <div className={`relative z-50 flex items-center justify-between px-4 sm:px-8 py-3 text-white animate-fade-in-up ${autoReportBanner === "sending" ? "bg-gradient-to-r from-blue-500 to-blue-600" :
          ...
          {/* Risk Score Bar — always visible below header */}
          <div className={`flex items-center justify-between px-4 sm:px-8 py-2.5 border-b transition-all duration-700 ${riskScore > 60 ? 'bg-red-50/80 border-red-100' :
            riskScore > 30 ? 'bg-amber-50/80 border-amber-100' :
              'bg-emerald-50/60 border-emerald-100'
            }`}>
            <div className="flex items-center gap-3">
              <div className={`relative flex h-3 w-3`}>
                <span className={`absolute inline-flex h-full w-full rounded-full opacity-50 ${riskScore > 60 ? 'bg-red-500 animate-ping' :
                  riskScore > 30 ? 'bg-amber-500 animate-ping' :
                    'bg-emerald-500'
                  }`}></span>
                <span className={`relative inline-flex h-3 w-3 rounded-full ${riskScore > 60 ? 'bg-red-500' :
                  riskScore > 30 ? 'bg-amber-500' :
                    'bg-emerald-500'
                  }`}></span>
              </div>
              <span className={`text-[12px] font-bold uppercase tracking-widest ${riskScore > 60 ? 'text-red-600' :
            ...
            <div className="flex items-center gap-3">
              <div className="w-32 sm:w-48 h-2 rounded-full bg-black/5 overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-1000 ease-out ${riskScore > 60 ? 'bg-gradient-to-r from-red-400 to-red-500' :
                    riskScore > 30 ? 'bg-gradient-to-r from-amber-400 to-amber-500' :
                      'bg-gradient-to-r from-emerald-400 to-emerald-500'
                    }`}
                  style={{ width: `${riskScore}%` }}
                />
              </div>
              <span className={`text-[13px] font-extrabold tabular-nums min-w-[3ch] text-right ${riskScore > 60 ? 'text-red-600' :
                riskScore > 30 ? 'text-amber-600' :
                  'text-emerald-600'
                }`}>
                {riskScore}
              </span>
              <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${riskScore > 60 ? 'bg-red-500 text-white' :
                riskScore > 30 ? 'bg-amber-500 text-white' :
          ...
          <div className="mt-4 text-center text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400">
            ...
            {messages.map((message, index) => (
              ...
              >
                ...
                <div className="flex max-w-[80%] flex-col">
                  ...
                  <div
                    className={`rounded-2xl px-5 py-3.5 text-[15px] leading-relaxed transition-all duration-700 shadow-sm ${message.role === "user"
          ...
          <div className="mx-4 sm:mx-8 mb-3 flex flex-wrap items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-3">
              ...
              <button
                ...
                className={`rounded-full px-4 py-2 text-[12.5px] font-semibold border shadow-sm transition-all hover:scale-105 active:scale-95 ${sleepQuality === "GOOD"
              ...
              <button
                ...
                className={`rounded-full px-3 py-2 text-[12.5px] font-semibold border shadow-sm transition-all hover:scale-105 active:scale-95 ${shareLocation
              ...
              <div className="flex items-center justify-between px-2 pb-1">
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className={`flex h-10 w-auto px-4 items-center justify-center rounded-xl transition-all font-medium text-[13px] ${voiceMode
          ...
          <div className="relative flex flex-col items-center gap-10">
            ...
            <div
              ...
              style={{
                ...
                background: `radial-gradient(circle, ${breathPhase === "inhale" ? "rgba(56, 189, 248, 0.15)" :
            ...
            <div
              ...
              style={{
                ...
                transitionDuration: breathPhase === "inhale" ? "4s" : breathPhase === "hold" ? "0.3s" : "4s",
                boxShadow: `0 0 60px ${breathPhase === "inhale" ? "rgba(56, 189, 248, 0.4)" :
            ...
            <div className="flex items-center gap-3 mt-2">
              {["inhale", "hold", "exhale"].map((phase) => (
                <div
                  key={phase}
                  className={`h-2.5 w-2.5 rounded-full transition-all duration-500 ${breathPhase === phase
